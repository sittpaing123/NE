...
from clone_plugins.info import ADMINS, LOG_CHANNEL
from clone_plugins.database.ia_filterdb import save_file
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
from clone_plugins.utils import temp
    ...
    async with lock:
        try:
            ...
            async for message in bot.iter_messages(chat, lst_msg_id, Cache.CURRENT):  # type: ignore
                if Cache.CANCEL:
                    inserted, errored = await save_file.insert_pending()
                    if inserted:
                ...
                inserted, errored = await save_file.insert_many(media)
                if inserted:
                    total_files += inserted
                if errored:
                    duplicate += errored
